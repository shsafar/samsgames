//
//  WebHashtagWordsGameView.swift
//  Sam's Games
//
//  Hashtag Words game integrated with daily puzzle system
//

import SwiftUI
import WebKit

struct WebHashtagWordsGameView: View {
    ...
    private var isAlreadyCompleted: Bool {
        ...
        return dailyPuzzleManager.isCompletedToday(.hashtagWords)
    }
    ...
    init(archiveMode: Bool = false, archiveDate: Date? = nil, archiveSeed: Int? = nil) {
        ...
        if let archiveSeed = archiveSeed, let archiveDate = archiveDate {
            ...
            self.level = manager.getHashtagWordsLevelForDate(archiveDate)
        } else {
            self.seed = manager.getSeedForToday()
            self.level = manager.getTodayHashtagWordsLevel()
        }
    }
    ...
    var body: some View {
        ...
        .alert("Puzzle Completed!", isPresented: $showCompletionAlert) {
            ...
            Text("Great job! You completed today's Hashtag Words puzzle!")
        }
        .sheet(isPresented: $showInstructions) {
            GameInstructionsView(gameType: .hashtagWords)
        }
    ...
    private var splashScreen: some View {
        ZStack {
            ...
            VStack {
                ...
                if let customIcon = GameType.hashtagWords.customIcon,
    ...
    private var gameView: some View {
        VStack(spacing: 0) {
            ...
            WebHashtagWordsGameViewRepresentable(
                seed: seed,
                level: level,
                onGameComplete: {
...
struct WebHashtagWordsGameViewRepresentable: UIViewRepresentable {
    let seed: Int
    let level: Int
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        // Load local hashtagwords.html
        if let htmlPath = Bundle.main.path(forResource: "hashtagwords", ofType: "html") {
            let url = URL(fileURLWithPath: htmlPath)
            webView.loadFileURL(url, allowingReadAccessTo: url.deletingLastPathComponent())
            print("Loading Hashtag Words game from: \(htmlPath)")
        } else {
            print("Error: hashtagwords.html not found in bundle")
        }
    ...
    class Coordinator: NSObject, WKNavigationDelegate, WKScriptMessageHandler {
        var parent: WebHashtagWordsGameViewRepresentable
        var onGameComplete: () -> Void

        init(_ parent: WebHashtagWordsGameViewRepresentable, onGameComplete: @escaping () -> Void) {
            self.parent = parent
        ...
        func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
            ...
            // Set seed and level, then select level and load puzzle
            let script = """
            ...
            if (window.setSeed && window.selectLevel) {
                ...
                console.log('Hashtag Words started at level \(parent.level)');
            } else {
            ...
            webView.evaluateJavaScript(script) { result, error in
                if let error = error {
                    ...
                    print("Hashtag Words initialized successfully")
                }
            }
        }
...
struct HashtagWordsCompletedView: View {
    @Environment(\.dismiss) private var dismiss
    @Environment(\.colorScheme) var colorScheme
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 30) {
                    ...
                    VStack(spacing: 12) {
                        ...
                        Text("Great job! You've finished today's Hashtag Words puzzle.")
                            .font(.system(size: 18))
